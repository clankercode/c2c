...
### 5. Defensive slices on recently-fixed code paths need heightened peer-PASS rigor

Cedar's #611 (Phase 2C, the MAIN_TREE cache) illustrates this precisely:

The slice targeted `git-shim.sh` — the **exact same code** that stanza's #613 had just fixed
hours earlier. The OCaml codegen that generates the installed `git-pre-reset` shim wrote
backslash-escaped quotes (`\"...\"`) inside a bash `MAIN_TREE="$(...)"` command substitution.
Combined with the shim's `set -euo pipefail`, this caused an `unbound variable` error on
parameter expansion, killing **all** git-via-shim operations. Coordinator1 hit this ~6 minutes
after the slice landed — in the process of verifying the fix, the shim regression caused
git timeouts across all managed sessions.

**Mechanism:** The OCaml `dune exec` codegen (in `just codegen-git-pre-reset` or equivalent)
unescaped the `\"` sequences when emitting shell code, producing invalid bash syntax.

**Mitigation (peer-PASS rubric for shim-modifying slices):** When a slice modifies
the generated shim or its OCaml codegen, the peer-PASS checklist must include a
**shell-strict-mode probe**:
```bash
# Verify the generated shim passes bash strict-mode syntax check
bash -n <(cat <<'GENERATED_SHIM'
# ... emit the generated shim here via the same codegen path ...
GENERATED_SHIM
)

# Verify the generated shim sources cleanly under set -euo pipefail
bash -c 'set -euo pipefail; source /dev/stdin' < <(cat <<'GENERATED_SHIM'
# ... emit the generated shim here ...
GENERATED_SHIM
)
```
Without this probe, a codegen bug in a shim-modifying slice can re-introduce the exact
failure mode that #613 fixed — in the same blast radius, within minutes of landing.

---

## Addendum: Cedar #611 Regression (2026-05-02, ~T+50min)

Cedar's #611 feat commit (`6086858e`, later cherry-picked as `395fc0ef`) introduced
a regression in the `git-pre-reset` shim generated by OCaml codegen. The generated
shim contained:
```bash
...
MAIN_TREE="$(git rev-parse --git-common-dir 2>/dev/null | xargs dirname || echo '')"
```
The double-quoted empty-string (`echo \"\"`) is invalid inside a command substitution
when `set -euo pipefail` is active and the `echo` produces no output — the `\"` is
treated as literal characters, not an empty-string literal.

**Impact:** All subsequent `git` invocations via the shim died with `unbound variable`.

**Detection:** Coordinator1's own `git` operations timed out within ~6 minutes of
the slice landing.

**Resolution:** Coordinator1 reverted (`bcc67021 Revert "feat(#611): cache MAIN_TREE..."`),
hand-patched the installed shim state path, and re-tasked cedar.

**Lesson:** The same shell path that #613 protected can be broken again by a
downstream change to the same file or its codegen. The emergency fix + the defensive
slice were both committed within the same hour; the speed of the response cycle
compressed review time and missed the codegen integration check.

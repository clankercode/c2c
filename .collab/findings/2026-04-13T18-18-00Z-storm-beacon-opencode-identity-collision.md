...
- Fix status: in progress. The checked-in repo-local config is separated from the managed `opencode-local` identity, and the managed launcher is being switched to a dedicated OpenCode config via `OPENCODE_CONFIG` so it no longer partially inherits the generic repo config.

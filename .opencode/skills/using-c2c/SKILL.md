...
- c2c self-configures via `c2c install <client>` — run this once per client. After installing, restart your CLI client (or run `/reload-plugins` in Claude Code) and resume the session. This activates push-based hook delivery — far more reliable than manual polling.

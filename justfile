...
# Regenerate ocaml/cli/c2c_claude_skill_embedded.ml from the canonical
# c2c skill markdown. `.collab/skills/` is the single tracked skill source
# used by both `just sync-skills` fan-out and the embedded Claude install blob.
# The binary embeds the skill so `c2c install claude` writes
# ~/.claude/skills/c2c/SKILL.md without needing the repo present.
# Run this recipe after editing .collab/skills/c2c.md and commit both
# the .md source and the generated .ml.
codegen-claude-skill:
    #!/usr/bin/env bash
    set -euo pipefail
    src=.collab/skills/c2c.md
    out=ocaml/cli/c2c_claude_skill_embedded.ml
    delim=claude_skill_src
    if [ ! -f "$src" ]; then
        echo "error: $src not found" >&2
        exit 1
    fi
    ...
    {
        printf '(* AUTO-GENERATED by `just codegen-claude-skill` from\n'
        printf '   .collab/skills/c2c.md. Do not edit by hand. *)\n\n'
        printf 'let content = {%s|' "$delim"
        cat "$src"
        printf '|%s}\n' "$delim"
    } > "$out.tmp"

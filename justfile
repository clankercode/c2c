...
# Claude skill markdown. The binary embeds the skill so `c2c install claude`
# writes ~/.claude/skills/c2c/SKILL.md without needing the repo present.
# Run this recipe after editing data/claude-skill/SKILL.md and commit both
# the .md source and the generated .ml.
codegen-claude-skill:
    #!/usr/bin/env bash
    set -euo pipefail
    src=data/claude-skill/SKILL.md
    ...
    fi
    {
        ...
        printf '   data/claude-skill/SKILL.md. Do not edit by hand. *)\n\n'
        printf 'let content = {%s|' "$delim"
        cat "$src"
        printf '|%s}\n' "$delim"
    } > "$out.tmp"

...
# Regenerate ocaml/cli/c2c_hermes_plugin_embedded.ml from the canonical
# Python plugin source at data/hermes-plugin/. The binary embeds all plugin
# files so `c2c install hermes` works without the repo present. Edit the
# sources under data/hermes-plugin/, run this, and commit BOTH the sources and
# the generated .ml — the CI drift gate re-runs this and diffs.
codegen-hermes-plugin:
    #!/usr/bin/env bash
    set -euo pipefail
    src_dir=data/hermes-plugin
    out=ocaml/cli/c2c_hermes_plugin_embedded.ml
    {
        printf '(* AUTO-GENERATED by `just codegen-hermes-plugin` from\n'
        printf '   data/hermes-plugin/. Do not edit by hand. *)\n\n'
        printf 'let files : (string * string) list = [\n'
        idx=0
        for f in \
            plugin.yaml \
            __init__.py \
        ...
        do
            ...
            # OCaml string delimiters cannot contain digits; use letters only.
            letters="abcdefghijklmnopqrstuvwxyz"
            first_idx=$((idx / 26))
            second_idx=$((idx % 26))
            if [ "$first_idx" -eq 0 ]; then
                suffix="${letters:$second_idx:1}"
            else
                first_letter="${letters:$((first_idx - 1)):1}"
                second_letter="${letters:$second_idx:1}"
                suffix="${first_letter}${second_letter}"
            fi
            delim="hermes_src_${suffix}"
            if grep -Fq "|$delim}" "$src"; then
                echo "error: $f contains the OCaml string delimiter '|$delim}'" >&2
                echo "       regenerate with a different delimiter scheme." >&2
                exit 1
            fi
            printf '  ("%s", {%s|' "$f" "$delim"
            cat "$src"
...
build: codegen-role-designer codegen-opencode-plugin codegen-hermes-plugin codegen-claude-skill codegen-alias-words codegen-changelog
...
build-cli: codegen-role-designer codegen-opencode-plugin codegen-hermes-plugin codegen-claude-skill codegen-alias-words codegen-changelog
...
build-server: codegen-role-designer codegen-opencode-plugin codegen-hermes-plugin codegen-claude-skill codegen-alias-words codegen-changelog
...
install-all: codegen-role-designer codegen-opencode-plugin codegen-hermes-plugin codegen-c2c-skills

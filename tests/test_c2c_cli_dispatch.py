...
class C2CCLIDispatchTests(unittest.TestCase):
    ...
    def test_send_works_with_agent_session_id(self):
        ...
        that managed peers can still invoke `c2c send` even when they have
        their own C2C_MCP_SESSION_ID set (i.e., the OC plugin process).

        We use --help which exercises the command dispatch without requiring
        a live broker or registered peers.
        """
        ...
        env.pop("C2C_REGISTRY_PATH", None)
        env.pop("C2C_ALIAS_WORDS_PATH", None)
        ...
        self.assertEqual(
            result.returncode,
            0,
            msg=f"c2c send --help failed with C2C_MCP_SESSION_ID set: {result.stderr}",
        )

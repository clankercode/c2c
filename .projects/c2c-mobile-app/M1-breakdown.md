...
> **Invariant**: `nonce` is **always emitted** in the JSON envelope — never omitted. It is `null` when the message is `enc:"plain"` (nobox), and a base64url 24-byte random string when `enc:"box-x25519-v1"`. This ensures deterministic shape for canonical JSON signing across all clients.
>
> **JSON string escaping**: all leaf strings (keys and `String` values) use `JSON.stringify`-compatible escaping via `Yojson.Safe.to_string (`String s)`. Do NOT use `Printf.sprintf "%S"` — OCaml's `%S` format uses OCaml escape conventions (`\n`, `\t`, etc.) which diverge from JSON for control chars, some unicode, and the `\\/` escapesequence. This was a latent cross-client interop bug; fixed in `relay_e2e.ml` canonical_json.

- Sender resolves recipient's `x25519_pubkey` via relay (cached); if present, `Hacl.NaCl.box ~pt:body ~n:nonce ~pk:recipient_x25519_pub ~sk:sender_x25519_priv` per recipient (NaCl crypto_box semantics: X25519 → HSalsa20 → XSalsa20-Poly1305). Nonce is fresh 24B random per recipient per message. Otherwise plaintext + warn log. For multi-recipient sends (rooms M4), use `Hacl.NaCl.box_beforenm` to precompute shared-key per recipient once, then `box_afternm` per message.

...
| Auto-delivery mechanism | PostToolUse hook (`c2c-inbox-hook-ocaml`) | Codex hooks (`c2c hook codex` via UserPromptSubmit/PostToolUse/SessionStart/SessionEnd) for vanilla and managed sessions; hooks fire on session activity, so explicit polling remains the idle fallback | `pi-c2c` extension: `fs.watch` (inotify) on broker inbox -> `pi.sendMessage` | c2c.ts plugin -> `promptAsync` | Notification-store (`C2c_kimi_notifier`) |
...
| deliver daemon | ✅ via PostToolUse hook (hook IS the daemon) | ✅ pre-trusted Codex hooks (vanilla + managed) | ✅ inotify `fs.watch` + hardcoded 60s safety-net poll | ✅ `c2c.ts` monitor subprocess | ✅ `C2c_kimi_notifier` writes notification files + tmux idle-wake |

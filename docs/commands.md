...
| `forward-agent-log --file SESSION_JSONL [--format claude] [--from ALIAS] [--interval SECS] [--max-bytes N] [--from-start] [--once] [--dry-run] TO` | Follow a coding-agent session transcript (jsonl) and forward only the human-visible conversation to a c2c peer: user input as `[user] …`, assistant plaintext as `[agent] …`. Filters out tool calls/results, thinking blocks, system/meta/summary events, hook and system-reminder injections, local-command output echoes, subagent (sidechain) lines, and c2c-envelope-delivered messages; malformed or partially-written lines are skipped (only newline-complete lines are consumed). Default starts at end-of-file so attaching to a long session does not flood the recipient — `--from-start` replays history, `--once` processes the current file and exits, `--max-bytes` (default 2000) truncates long messages UTF-8-safely with a `[truncated: …]` note. `TO` uses the same addressing as `c2c send` (local alias or `alias@host`). Format support: `claude` (Claude Code session files under `~/.claude*/projects/<slug>/<session-id>.jsonl`); other clients' formats can be added later. Intended for observation/monitoring, e.g. mirroring a session to a colleague's agent on another machine; forwarded content is plain message DATA (no approval/RPC semantics). |

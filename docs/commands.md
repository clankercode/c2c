...
CLI equivalent: `c2c server-info [--json]` (see Diagnostics & maintenance below).
...
- `show <name>` shows the details of a single schedule entry (not covered by
  the MCP `schedule_list` tool, which only returns all entries).
...
`init`, `install`, and `uninstall` are **Tier 2** (lifecycle/setup — visible with care in agent sessions; match `command_tier_map` in source). Prefer operator intent for `install`/`uninstall` even though they are not Tier-3-hidden. Client MCP is never installed by default — pass `--with-mcp` to `c2c install <client>` (or `c2c init --with-mcp`) only when deliberately enabling MCP. A bare `c2c install <client>` writes hooks + the `/c2c` skill (and, for OpenCode, the plugin), not MCP. CLI messaging (`c2c send` / `c2c monitor` / `c2c poll-inbox`) works without MCP.
...
| `install all [--with-clients] [--with-mcp]` | 2 | Scriptable binary install only by default. Does **not** configure any client unless `--with-clients` (explicit bulk opt-in), and `--with-mcp` additionally enables MCP config for those selected clients. Prefer `c2c install <client>`. |
| `install claude\|codex\|codex-headless\|opencode\|kimi\|grok\|agy\|hermes [--alias A] [--broker-root DIR] [--with-mcp] [--dry-run]` | 2 | Configure one client for c2c messaging. By default writes hooks + the `/c2c` skill (and, for OpenCode, the plugin) — no MCP; pass `--with-mcp` to also write the client's MCP server config and its MCP env for auto-register/auto-join. Without MCP, the hooks/plugin still provide delivery and session registration, but room membership is explicit through the skill/CLI. `grok` is CLI-first (skill + SessionStart/SessionEnd hooks under `~/.grok/`, no MCP); `agy` — Antigravity CLI: embedded skill + SessionStart/PostToolUse/Stop hooks under `~/.gemini/`, agentapi wake delivery, no MCP. `hermes` is plugin-based and has **no** `--with-mcp` path: it writes the embedded Python plugin + `skills/c2c/SKILL.md` to `~/.hermes/plugins/c2c/` and adds `c2c` to `plugins.enabled` in `~/.hermes/config.yaml` (restart Hermes so the plugin loads). `kimi` appends managed blocks (including the `c2c hook kimi` SessionStart hook) to `~/.kimi-code/config.toml`, writes the `/c2c` skill to `~/.kimi-code/skills/c2c/SKILL.md`, and installs `~/.local/bin/c2c-kimi-approval-hook.sh` (the `~/.kimi-code/mcp.json` entry is written only with `--with-mcp`). `claude` also wires hooks into `~/.claude/settings.json`: PostToolUse (drain), Stop (text-only-turn delivery), and SessionStart/SessionEnd (`~/.claude/hooks/c2c-session-hook.sh` running `c2c hook claude` — onboarding/wake text, cold-boot + post-compact context, message drain, deregister-on-end). `claude` and `codex` also install the embedded `/c2c` skill (`~/.claude/skills/c2c/SKILL.md` / `~/.codex/skills/c2c/SKILL.md`; both copies auto-refresh on SessionStart via the c2c hooks). Replaces the legacy per-client `configure-*` subcommands. On success, prints a consolidated "Installed c2c for `<component>`" summary with owned/shared artifacts and a `c2c uninstall <component>` hint. (`install crush` still routes for legacy cleanup but prints `[DEPRECATED]` and is not a supported client.) |
...
| `uninstall kimi [--alias A]` | 2 | Remove `mcpServers.c2c` from `~/.kimi-code/mcp.json`, the approval-hook block from `~/.kimi-code/config.toml`, and owned files — including both skills, `~/.kimi-code/skills/c2c/SKILL.md` and `~/.kimi-code/skills/c2c-session/SKILL.md`. Since #83 the `c2c-session` skill is not deleted at session end, so uninstall is the only thing that removes it. |
...
| `uninstall hermes` | 2 | Remove the Hermes plugin files under `~/.hermes/plugins/c2c/` (including `skills/c2c/SKILL.md`) and the `c2c` entry in `plugins.enabled` in `~/.hermes/config.yaml`. Directories are removed only when empty, so a live `__pycache__/` (Hermes byte-compiles the plugin on import) leaves the directory behind — inert once the sources are gone. |
...
| `uninstall all` | 2 | Uninstall every component above — including `agy` and `hermes` — in the sweep (clients first, then git pieces, then `self` last). |
...
| `list [--all] [--alive] [--match SUBSTR] [--kind local\|relay] [--global] [--relay] [-e\|--enriched] [--relay-alias ALIAS] [--json] [--cross-repo]` (alias: `peers`) | List registered peers. Default output hides only registrations confirmed `dead`, including relay rows; `unknown` remains visible because a PID-less or unverifiable client may still receive on its next hook. **Default-broker cwd-scope filter (#74):** when the listing's broker root is the shared `default` fingerprint broker, rows whose registration `cwd` is outside the current scope directory — the git toplevel when inside a repo, else the current working directory — are hidden so unrelated non-repo agents (hundreds, on a busy host) stop looking like peers. Real repo brokers are already fingerprint-partitioned and are **not** filtered this way (same-repo worktrees would otherwise hide each other). A subdirectory of the scope dir counts as in-scope; a row with **no** `cwd` metadata fails open (always shown) so a possibly-live peer is never hidden for lack of metadata. When rows are hidden this way a `(N agents in other directories hidden — use --all to show)` line is printed to stderr (in both human and `--json` mode, so `--json` stdout stays clean). `--all` adds session ID + registered time, restores confirmed-dead rows for diagnostics, **and disables the default-broker cwd-scope filter** (shows every agent in the broker regardless of `cwd`); `--alive` is the strict alive-only subset. `--match SUBSTR` filters by case-insensitive alias substring (composes with the other flags, but does not reveal dead rows without `--all`). `-e`/`--enriched` shows role-class + description + last-seen for each peer (looked up from `.c2c/roles/<alias>.md`). `--relay-alias ALIAS` sets the alias to sign the relay `/list` request as for `--relay` (default: `C2C_MCP_AUTO_REGISTER_ALIAS`, else an `anon` placeholder); must be bound to your local identity on the relay via `c2c relay register --alias ALIAS`. Liveness is `alive`, `dead`, or `unknown`; `--json` emits both tri-state `alive` (`true`/`false`/`null`) and explicit `state` with that label. A vanilla Codex hook-only registration is `alive` while its bounded hook-activity lease is fresh, meaning a queued message can be delivered on its next hook; expired hook-only rows are filtered from discovery and `--alive`. `--global` scans all known broker roots system-wide and renders visible rows in repository groups (it spans brokers, so the repo-scope filter does not apply). `--cross-repo` targets the shared sessions broker (`~/.c2c/sessions/broker`) — an explicit cross-repo view, so it too bypasses the repo-scope filter. `--relay` merges configured relay peers with local rows, tagging each row with `source`, full `<alias>@<host_id>` address, `identity_pk`, liveness, and the identity labels `identity_kind` (`local` = session alias on this broker; `relay` = alias@host_id anchored to a machine key) and `identity_scope` (`local`\|`relay`\|`both`); a lease that is this machine's own registration (same alias + this host's host id) folds into its local row as one scope-`both` identity (JSON nests it under `relay_lease`), while the same alias on a different host stays a distinct row disambiguated by address. `--kind local\|relay` filters by identity kind/scope (scope-`both` rows pass both). Relay fetch failures are non-fatal: local rows still print, human mode adds a stderr note, `--json` wraps the merged rows in `{"peers": [...], "relay_error": null\|"..."}`, and the exit code stays 0 (partial success). The default (no `--relay`) listing stays local-only with a bare-array JSON; the merged-by-default flip is an open product gate (see `.collab/design/friction-cn-decision-ledger.md` on the `friction-adr0-decision-ledger` branch). See [Reference: identifiers](/reference/identifiers/#identity-kind-and-scope-in-the-merged-listing). |
...
| `rooms create ROOM [--visibility VIS] [--invite ALIAS]... [--no-join] [--alias A] [--json]` | Create a room with explicit visibility (#394). `--invite` pre-populates `invited_members` for `gated`/`private` rooms (repeatable; cross-host `alias@host` refused). `--no-join` creates without auto-joining as a member. See also `rooms visibility` below. |
...
| `rooms visibility ROOM [--set\|--visibility public\|unlisted\|gated\|private]` | Get or set room visibility. `--set` and `--visibility` are equivalent. `public` = listed + open join; `unlisted` = unlisted + open join; `gated` = listed + invite-gated join; `private` = unlisted + invite-gated join. To set visibility *and* pre-seed the invite ACL at creation time, use `rooms create` instead (above). |
| `rooms delete ROOM [--force] [--json]` | Delete an empty room. `--force` bypasses the legacy-room creator check (only honored when the room's meta has no recorded creator). |
...
| `stop NAME [--json]` | Stop a managed instance (SIGTERM the outer loop). **B302:** for `NAME=relay-connect`, if the `c2c-relay-connect.service` systemd --user unit owns the connector, the stop delegates to `systemctl --user stop c2c-relay-connect.service` (a direct kill is reverted by `Restart=always` within ~5s) with a loud notice; a rogue non-systemd supervisor holding the machine singleton while the unit is active is stopped directly (pid-identity-guarded) and systemd re-acquires on its next auto-restart; unit present-but-inactive or no systemd → direct path, and the recorded connector child is swept in every state. |
| `restart NAME [--timeout SECS]` | Stop then start a managed instance. For `NAME=relay-connect`, drives the machine-wide connector lifecycle (B212); if no managed config exists, bootstraps a supervised connector when a relay URL is known via `C2C_RELAY_URL` / `c2c relay setup` (B235) instead of "no config found". **B302:** when the `c2c-relay-connect.service` unit owns the connector, restart delegates to `systemctl --user restart c2c-relay-connect.service` (the unit re-resolves its own relay URL at start); with a rogue supervisor holding the singleton while the unit is active, the rogue is stopped first, then the restart is delegated. |
| `reset-thread NAME THREAD` | For `codex` / `codex-headless`, persist an exact resume target and restart onto that thread. |
| `restart-stale [--dry-run] [--exclude-coordinator] [--force] [--timeout SECS] [--json]` | Version-aware rolling restart of managed instances on an outdated `c2c` binary (I010/I011). App-server sessions restart in place via owner-control (Busy fail-closed unless `--force`; Unknown delegates to the owner status API). TUI/hook clients request in-pane outer owner-control when idle-eligible or `--force` (Busy/Unknown stay guided-manual). Never execves a supervisor into the caller TTY. Coordinator last unless `--exclude-coordinator`. |
| `restart-sidecar <name> <deliver|poker>` | Restart a managed **deliver** or **poker** sidecar without killing the inner client (I012). Refuses `notifier`. Local operator only (B098). |
...
| `health [--json]` | Broker health snapshot: registry liveness, inbox freshness, rooms, relay reachability, client plugin status, and shared-config file modes. The last (`shared_config_modes` in JSON) walks the install manifest's shared artifacts — the configs c2c writes *into* but does not own — and reports any that are world-writable. c2c never changes the mode of a file it did not create, so this reports and leaves the decision to you (#84). |
...
| `server-info [--json]` | Show c2c client version and feature flags. CLI counterpart to the MCP `server_info` tool. |
| `version` | Print the c2c version — identical to `c2c --version`. Primary line is the parseable build-time identity (`<ver> <sha> (built <date>)`) with a demarcated secondary `[now:]` clock for skew diagnostics; a trailing line notes a newer release when the local changelog cache already knows of one (cache-only, no network). |
| `deregister ALIAS [--cross-repo] [--json] [--broker-root DIR]` | Remove a registration from the broker. |
...
| `doctor hooks [--compact] [--json] [--fix]` | Check Claude Code `settings.json` hook entries for dangling c2c scripts, Codex managed-block drift, and the live Codex delivery mode (`app-server` / `app-server-unavailable` / `hooks+wake` / `hooks` / `unavailable`) with a remediation per degraded state. `--fix` restores dangling c2c-owned hook scripts (`c2c-inbox-check.sh`, `c2c-stop-deliver.sh`, `c2c-session-hook.sh`) to the paths `settings.json` references — rewriting them from the same source `c2c install claude` uses, without modifying `settings.json` (self-healing for the shared-hooks-dir orphan in #19). |
...
| `stop NAME [--json]` | Stop a managed instance. **B302:** `relay-connect` delegates to `systemctl --user stop c2c-relay-connect.service` when the unit owns the connector (direct kills are reverted by `Restart=always`); a rogue non-systemd supervisor holding the machine singleton while the unit is active is stopped directly (pid-identity-guarded, systemd re-acquires); unit inactive or absent → direct supervisor path, recorded connector child swept in every state. |
| `restart NAME [--timeout SECS]` | Stop then start a managed instance. `relay-connect` uses the machine lifecycle and B235-bootstraps when no config exists and a relay URL is known. **B302:** delegates to `systemctl --user restart c2c-relay-connect.service` when the unit owns the connector; a rogue singleton holder is stopped first, then the restart is delegated. |
| `reset-thread NAME THREAD` | Restart a managed codex/codex-headless onto a specific thread. |
| `restart-stale [--dry-run] [--exclude-coordinator] [--force] [--timeout SECS] [--json]` | Version-aware rolling restart (I010/I011). App-server: owner-control in place (Busy fail-closed; `--force` overrides). TUI/hook: outer owner-control when idle-eligible or `--force`; otherwise guided. No caller-TTY theft. Coordinator last unless `--exclude-coordinator`. |
| `restart-sidecar <name> <deliver|poker>` | Restart a managed **deliver** or **poker** sidecar without killing the inner client (I012). Refuses `notifier`. Local operator only (B098). |
| `statefile [--instance NAME] [--tail] [--json]` | Read or watch the OpenCode plugin state snapshot. |
...
| `schedule show NAME [--json]` | Show details of a single schedule entry. |
...
| `coord-cherry-pick [--no-dm] [--no-fail-on-install] [--no-install] SHA…` | Coordinator: cherry-pick SHAs with dirty-tree safety + install + author DM. Also available as `c2c coord cherry-pick` (same semantics, group form). |
...
| `gc-inboxes [--apply] [--older-than DURATION] [--root DIR] [--cross-repo] [--json]` | Reclaim inbox files that have **no registration row at all** (not dead, not pidless — absent). Dry-run by default: prints count, messages, age distribution, and by-sender breakdown, touching nothing; pass `--apply` to delete. An inbox is eligible only when it is BOTH row-less AND older than `--older-than` (file mtime and newest message ts both precede the cutoff — the race guard; default `7d`). Preserves every inbox that still has a registration — including live, dead, pidless, and managed rows (that is `sweep`'s territory) — and fails closed (reclaims nothing, exits 123) if the registry cannot be read as a JSON list. Distinct from `sweep`, which also deletes dead-row inboxes. On `--apply`, non-empty content is archived to `dead-letter.jsonl` with reason `inbox_gc` before unlink (archive-then-remove, same pattern as `sweep`); review the dry-run first. |
...
| Subcommand | Description |
|------------|-------------|
| `relay enable [--url URL] [--token T] [--unit-binary PATH]` | **Activate the relay on this host.** The relay is opt-in: c2c contacts no relay until this is run (or `C2C_RELAY_URL` / `--relay-url` is given). URL precedence: `--url` > `C2C_RELAY_URL` > an already-saved relay.json `url` (including one parked by `c2c relay disable`); the public relay `https://relay.c2c.im` is used only when none of those is set. Writes `url` + `enabled: true` to `relay.json`. Also installs + enables a `systemd --user` unit (`c2c-relay-connect.service`, `Restart=always`) so the connector survives reboots/logouts — skipped with a note on hosts without systemd --user. `--unit-binary PATH` pins the ExecStart binary (default: the canonical install `~/.local/bin/c2c`, else this executable); a dev `_build` binary is **refused** (a `dune clean` would leave the unit restarting a missing binary forever) unless overridden explicitly. |
| `relay disable` | Return this host to local-only. Sets `enabled: false` and **keeps** the configured URL, so `c2c relay enable` restores it. Stops + disables the boot-supervision unit (the unit file is kept). Best-effort sends the shutdown command to a running subscribe-daemon (B309), and prints one line naming both classes of lingering consumers (subscribe-daemon outcome; `c2c monitor` relay watchers keep their startup-resolved URL until restarted). Same-machine DMs, rooms and broadcast are unaffected. |
...
| `relay register --alias A [--relay-url URL] [--node-id N] [--session-id S] [--force]` | Register Ed25519 identity on the relay (prod-mode bootstrap). **B294:** refuses with an explanation when a live `relay-connect` connector owns the alias on this machine (read-only `connector-state.json` evidence) — registering would take the connector's lease and the two would fight; `--force` overrides, `c2c restart relay-connect` repairs instead, and `--node-id`/`--session-id` (or `C2C_RELAY_NODE_ID`/`C2C_RELAY_SESSION_ID`) deliberately register under a chosen key pair, e.g. to hand a lease back to the connector's keys. A `--node-id` alone implies node/node; `--session-id` alone is an error. |
...
| `relay dm poll [--alias A] [--node-id N] [--session-id S]` | Poll for cross-host DMs from the relay (drains the inbox). When a **live** `relay-connect` connector owns the alias lease, poll uses that connector (node_id, session_id) rather than `cli-<alias>` (B231) so it does not 403 after connector restart; B308 gates that preference on the same liveness evidence as the B294 register guard — a dead connector's stale key is never used, so after re-registering `cli-<alias>` you poll the key you registered. `--node-id`/`--session-id` (or `C2C_RELAY_NODE_ID`/`C2C_RELAY_SESSION_ID`, flags winning) override resolution with register's convention: a lone node id implies node/node, a lone session id is an error. Returns: `{"ok": true, "messages": [...]}` with each row a [schema-v1 message](#json-output-message-schema-v1) (`delivery.state:"delivered"`, `source:"relay"`) plus the legacy row keys; an empty batch keeps the exact legacy shape. Prefer a live connector for ongoing delivery; poll races the connector for the same inbox. |
| `relay dm peek [--alias A] [--node-id N] [--session-id S]` | Peek at pending cross-host DMs **without draining** the inbox (B096) — safe for monitor/tail watchers that must not steal messages from the poll consumer. Same B231/B308 connector-lease resolution as poll (live connector preferred, `--node-id`/`--session-id` overrides). Returns: same shape as `relay dm poll` but rows carry `delivery.state:"queued"`. |
| `relay dm send-all <message> --alias A` | Broadcast a message to relay peers (relay-side counterpart to top-level `send-all`). `--alias` is required (sender identity, not omittable). |
...
| `relay subscribe-daemon list [--mine]` | List aliases managed by the subscribe-daemon. **Default is daemon-global** (every open IPC client's aliases + `summary`: clients/aliases/connected/connecting/stopped). `--mine` limits to this CLI's IPC connection (almost always empty for one-shot list). Harness IPC `{"cmd":"list"}` stays per-client; pass `"all":true` or `list_all` for global (B278). |
| `relay subscribe-daemon shutdown` | Stop the subscribe-daemon |
| `relay rooms list [--alias A]` | List **public** and **gated** rooms on the relay (anonymous). With `--alias` (or auto-alias env), signs the request so **unlisted** rooms that alias is a member of also appear (B230). Gated rows keep `member_count` but redact `members` to `[]` for everyone on this directory surface (B229). Private rooms are never listed. |
...
| `relay register --alias A [--relay-url URL] [--token T] [--node-id N] [--session-id S] [--force]` | Register Ed25519 identity on the relay (prod-mode bootstrap). Refuses when a live connector owns the alias (B294; `--force` to override, explicit `--node-id`/`--session-id` to choose the lease keys). |
| `relay dm send TO MSG\|poll\|peek\|send-all [--alias A]` | Send, poll (drain), non-destructive peek, or broadcast (`send-all`, requires `--alias`) of cross-host direct messages. Results use [message schema v1](#json-output-message-schema-v1) with legacy keys preserved (`accepted`/`delivered`/`queued` respectively, `source:"relay"`). |
...
| `registry-prune [-p PREFIX]... [-f\|--force] [-n\|--dry-run] [-j\|--json]` | Remove dead test registrations matching prefix patterns (default: `eph-, heal-, mon-, tmp-, test-, zombie-`; dry-run by default, `--force` to actually prune). |
| `list-glyphs [--compact]` | (dev) Emit the canonical c2c TUI glyph registry as JSON. |
...
```bash
c2c list --json
c2c list --all         # include other-directory + confirmed-dead rows (disables the #74 default-broker cwd-scope filter)
...
2. Per-client environment variable set by the host:
   ...
   - Kimi: `$KIMI_SESSION_ID` when supplied by Kimi Code; the SessionStart hook records and adopts the live session identity. Its MCP env is written only with `c2c install kimi --with-mcp`.

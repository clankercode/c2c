...
| `peer-pass sign\|send\|verify\|list\|clean` | Sign, send, and verify peer-PASS review artifacts. The broker enforces signature + TOFU pubkey pin on `peer-pass send` (H2 + H2b, 2026-04-28): forged-signature, missing-artifact, and pin-mismatched DMs are rejected with a generic error; details land in broker.log under `event:"peer_pass_reject"`. Pin store at `<broker_root>/peer-pass-trust.json`; rotate via `c2c peer-pass verify --rotate-pin`. See `.collab/design/SPEC-signed-peer-pass.md`. |

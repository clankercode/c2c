...
- **Codex** — `c2c install codex` writes Codex hooks for `UserPromptSubmit`, `PostToolUse`, `SessionStart`, and `SessionEnd`. Those hooks run `c2c hook codex`, which can auto-register the session, drain inbound broker messages, and surface them through `hookSpecificOutput.additionalContext`; explicit `poll_inbox` remains the fallback. The legacy `--xml-input-fd` XML sideband for interactive Codex is gone (current Codex removed the flag; the codex-headless bridge keeps its own XML fifo path). Use `c2c reset-thread <name> <thread>` to force a managed Codex instance onto an exact thread instead of `resume --last`.

...
- **Hermes Agent** — plugin-first peer via `c2c install hermes`, which writes an in-process Python plugin to `~/.hermes/plugins/c2c/` and enables it in `~/.hermes/config.yaml`. It registers a `hermes-*` alias, exposes `c2c_send` / `c2c_list` / room tools plus `/c2c-*` slash commands, and runs a background watcher that peeks the broker, injects inbound mail as a DATA envelope via `ctx.inject_message`, and drains only after a successful inject. No MCP and no `c2c start hermes` — Hermes runs its own loop, like Pi Agent. Idle wake is GUARANTEED for CLI sessions; gateway sessions (Telegram/Discord) have no CLI reference to inject into, so mail stays queued for `c2c poll-inbox`.
...
2. **MCP tool path** (optional integration) — after MCP setup, agents call `mcp__c2c__send`, `mcp__c2c__poll_inbox`, and related tools. Claude Code, Codex, OpenCode, and Kimi can use MCP directly; Grok defaults to CLI only, as does agy (Google Antigravity — CLI-first, no MCP); Pi Agent and Hermes have no MCP path at all and drive the same broker through the `c2c` CLI, wrapped by the `pi-c2c` extension and the Hermes plugin respectively.

3. **Client-native delivery hooks/plugins** (optional integration) — Claude hooks, Codex hooks, Pi extension, OpenCode plugin, Hermes plugin (`ctx.inject_message`), Kimi REST prompt injection (`C2c_kimi_notifier`), Grok skill/hooks, and agy skill/hooks (agentapi inject via the deliver-watch sidecar) make inbound messages appear in client-specific surfaces. Message bodies remain broker-native; these integrations are convenience layers over the same inbox model. They vary in whether they can wake an idle agent — see [Delivery & Wake Contract](/wake-contract/).
...
Writes the Grok `/c2c` skill plus SessionStart/SessionEnd hooks. CLI-first: no MCP by default; no managed `c2c start grok`. Open a new Grok session so SessionStart can auto-register, then arm `c2c monitor` (or poll with `c2c poll-inbox`).

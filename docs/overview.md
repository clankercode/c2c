...
- **Codex** — `c2c install codex` writes Codex hooks for `UserPromptSubmit`, `PostToolUse`, `SessionStart`, and `SessionEnd`. Those hooks run `c2c hook codex`, which can auto-register the session, drain inbound broker messages, and surface them through `hookSpecificOutput.additionalContext`; explicit `poll_inbox` remains the fallback. Hook delivery happens at hook boundaries (the session's next turn), not on arrival — for vanilla and managed sessions alike. Managed `c2c start codex` (or `c2c new codex` for a fresh thread) is the canonical way to run a Codex peer: on supported Codex it runs the session behind an authenticated loopback app-server with the stock remote TUI attached, with hooks as the automatic fallback for older Codex. App-server interactive delivery is landing now — its arrival-time, draft-safe delivery stack is proven at the library level, with the live supervisor wiring being wired in; until that is confirmed live, Codex sessions receive at the hook boundary. `c2c doctor hooks` reports the delivery mode a session actually has. Full contract and wiring status: [Per-Client Delivery § Codex](/client-delivery/#codex). The legacy `--xml-input-fd` XML sideband for interactive Codex is gone (current Codex removed the flag; the codex-headless bridge keeps its own XML fifo path). Use `c2c reset-thread <name> <thread>` to force a managed Codex instance onto an exact thread instead of `resume --last`.

...
| `test_kimi_opencode_cross_host.py` | `docker-compose.e2e-multi-agent.yml` | #674 — Kimi + OpenCode cross-host relay E2E. 8 tests total: registration, room join, bidirectional DMs, room message delivery, and echo roundtrip task. Cross-host targets use the current public form `<alias>@<host_id>`; the historical #686 relay suffix parsing fix is already landed in `09b4d871`. |

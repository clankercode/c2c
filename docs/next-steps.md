...
- **OpenCode native plugin `promptAsync` delivery** — Background poll + CLI drain fully working after critical JSON parse bug fix (da78130, 2026-04-14). `promptAsync` end-to-end not yet confirmed — live test in progress. See `.collab/findings/2026-04-14T01-45-00Z-storm-beacon-opencode-plugin-json-parse-bug.md`.
...
- **OpenCode plugin `drainInbox` JSON parse fix** ✓ — `drainInbox()` was always returning `[]` because `poll-inbox --json` outputs an envelope object `{"session_id":...,"messages":[...]}` but the plugin did `Array.isArray(parsed) → false → []`. Inbox WAS being drained (CLI subprocess ran) but messages were silently discarded before `promptAsync` or the spool could see them. Fixed with `parsePollResult()` that unwraps the envelope. Also added `c2c peek-inbox` (non-destructive CLI subcommand), `file_fallback_peek()`, and 4 new tests (650 total). Commits: 8009020, da78130, 2026-04-14.

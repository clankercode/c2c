...
- **Kimi Wire bridge implementation** ✓ — `c2c_kimi_wire_bridge.py` + `c2c-kimi-wire-bridge` wrapper. `WireState`, `WireClient`, `C2CSpool`, `deliver_once`, CLI dry-run, `run_once_live`, and persistent `run_loop_live` daemon mode with exponential error backoff. 34 tests (2026-04-14).

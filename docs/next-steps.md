...
- **`c2c status`** ✓ — compact swarm overview for agent orientation after resume/compaction. Shows alive peers, broker-archive sent/received counts, goal_met state, dead registration count, and room membership summaries. Includes `last=Xs/Xm/Xh ago` per peer. Room lines now show alive member aliases inline; empty rooms (0 members) are hidden from text output. Goal line shows "Blocked by <alias>: need N more sends/recvs" for peers not yet at goal_met. 9 status tests; suite 856 total (1bf69c2 + f59f62f + d38396d + 85b7720 + b00ca66 + bd6bb4e, 2026-04-14, storm-beacon).
- **`c2c health` stale-inbox check** ✓ — `check_stale_inboxes()` scans `*.inbox.json` files and reports sessions with ≥5 pending messages (alias, count, hint). Integrated into `run_health_check()`. 7 Python tests; suite 847 (c85f057, 2026-04-14, storm-beacon).

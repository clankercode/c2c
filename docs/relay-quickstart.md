...
> **Security properties at a glance:** production relays authenticate peer
> requests with Ed25519; private recipients are consent-gated (no ordinary list/
> pubkey enumeration; first-contact needs a contact grant); encryption and TLS
> remain opportunistic/URL-selected. Full rundown: [Security](/security/) and
> [Connect → Security & privacy](/connect/#relay-security).
...
| `poll_inbox`/`heartbeat` 404 `lease_not_found` ("no live lease for session") | The `(node_id, session_id)` pair has no live lease — it expired or was registered under a different key (B293: distinct from `signature_invalid`, which means the signer does not own a lease someone else holds) | Nothing to fix by hand: `relay-connect` drops the dead registration and re-registers on the same pass. For a CLI-only lease, `c2c relay register --alias A` — but **not** while a connector manages the alias (that steals it; the command refuses, B294) |
| `c2c relay register` refuses with "relay-connect appears to own alias" | A live machine connector holds that alias's lease; registering under `cli-<alias>` would take it and the two would fight (B294) | `c2c restart relay-connect` to have the connector re-register, `c2c relay dm peek --alias A` to probe read-only, `--force` to take the lease deliberately, or `--node-id`/`--session-id` to register under chosen keys |

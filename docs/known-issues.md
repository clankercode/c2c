...
**Current path:** `c2c start kimi` spawns the kimi notifier (`C2c_kimi_notifier`), which discovers the session id from `~/.kimi-code/session_index.jsonl` and POSTs inbound messages as user prompts to the Kimi Code local REST server (`/api/v1/sessions/{id}/prompts`). That REST inject is the wake — no tmux required (CONDITIONAL on the notifier daemon being alive). Optional legacy TUI composer nudge only with `C2C_KIMI_TMUX_COMPOSER_WAKE=1` (default off). No PTY injection, no wire bridge. The legacy file-based notification-store is deprecated. Unmanaged plain `kimi` sessions: SessionStart (`c2c hook kimi`) best-effort arms a per-alias notifier and writes a `c2c-session` skill nudge; if no notifier is running, fall back to `c2c monitor` / `c2c poll-inbox`. SessionStart also non-destructively peeks the session inbox and, when messages arrived before the session started, renders a `📬 You have N c2c message(s) already queued … c2c poll-inbox` callout in that skill so the pre-startup backlog is visible immediately (#12). `c2c doctor hooks` flags DEAF sessions (undelivered inbox + no notifier — B238).

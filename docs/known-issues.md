...
**Fix:** Use `prune_rooms` for safe room cleanup, or check `pgrep -a -f "run-(kimi|codex|opencode|claude)-inst-outer"` before sweeping. See `c2c sweep-dryrun` for a read-only preview.

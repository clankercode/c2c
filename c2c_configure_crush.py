...
def write_crush_config(
    ...
    # Crush has no native session ID env var; use alias as stable session ID
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        if result["ok"]:
            ...
            print("  Tier 1 (manual TUI): start the wake daemon alongside Crush:")
            print("    nohup c2c-crush-wake --terminal-pid <ghostty-pid> --pts <pts> &")
            print("  Tier 2 (managed harness):")
            print("    1. Create run-crush-inst.d/<name>.json with command/cwd/c2c_alias")
            print("    2. Run: ./run-crush-inst-outer <name>")
        else:

...
- **Build + install via `just`.** OCaml changes are NOT live until the binary is rebuilt AND copied to `~/.local/bin/c2c`. Use the `just` recipes — atomic, handle "Text file busy", install all binaries together:
  ...
  ```
  Iterative dev: `just build` (compile check, narrow target list), `just check` (full `dune build` — catches latent broken test exes that the narrow `just build` skips), `just test` (full suite), `just test-one -k "pattern"`. `just --list` for everything. Run `just check` before any peer-PASS request or coord cherry-pick batch — it's how peer-PASS verifies "no broken sibling target" and the coord-side `install-all` does not (#432 receipt: test_relay_enc rotted for ~2 days behind narrow-target builds before Slice E surfaced it). Reach for raw `opam exec -- dune build` only if a recipe is missing — if you do, add a recipe.

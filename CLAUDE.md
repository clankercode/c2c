...
- **Git workflow — read `.collab/runbooks/git-workflow.md`.** That doc is the canonical reference. Five rules in short: (1) **one slice = one worktree** under `.worktrees/<slice-name>/` — never mutate the main tree for slice work; (2) **branch from `origin/master`** (NOT local master, which may contain unmerged peer work); (3) **real peer-PASS before coord-PASS** — another swarm agent runs `review-and-fix` on your SHA, and the reviewer's "build clean" verdict MUST come from a build run **inside the slice's own worktree** with the rc captured in the artifact's `criteria_checked` list (e.g. `build-clean-IN-slice-worktree-rc=0`) — see `.collab/runbooks/worktree-discipline-for-subagents.md` Pattern 8 (#427); **self-review-via-skill is NOT a peer-PASS**, and a subagent of yours doesn't count either; (4) **new commit for every fix, never `--amend`** — peer-PASS DMs reference the SHA, amend breaks the trail; (5) **coordinator gates all pushes** to origin/master. After cherry-picks verify HEAD is on a named branch (`git branch --show-current`); if blank, `git switch <branch>` before your next commit. Companion runbooks: `.collab/runbooks/worktree-per-feature.md` (worktree mechanics + `--worktree` flag), `.collab/runbooks/branch-per-slice.md` (slice sizing, drive-by discipline). The worktree-discipline runbook now catalogs **17 patterns** (Patterns 15, 17, and 18 added 2026-04-29); Patterns 6/13/14/15 cover the destructive-git-op family that the pre-reset shim guards. Slicing handoff example: `.collab/updates/2026-04-25T09-58-27Z-lyra-coordinator-handoff.md`.

- **Pre-reset shim refuses destructive main-tree ops (#452, 2026-04-29).** A wrapper installed swarm-wide via `c2c install` (v2) intercepts `git reset --hard <ref>`, `git checkout -- .`, and similar in the main tree and refuses for non-coord agents — preventing the "I'll just reset to clean up" footgun that wipes peers' staged work. Escape hatch: `C2C_COORDINATOR=1` for the coordinator role. If the shim refuses your op, that's the signal to switch to a worktree (or DM coord if you genuinely need the reset). Cross-link: `.collab/runbooks/worktree-discipline-for-subagents.md` Patterns 6/13/14/15.
...
- **Build + install via `just`.** OCaml changes are NOT live until the binary is rebuilt AND copied to `~/.local/bin/c2c`. Use the `just` recipes — atomic, handle "Text file busy", install all binaries together:
  ...
  ```
  Iterative dev: `just build` (compile check, narrow target list), `just check` (full `dune build` — catches latent broken test exes that the narrow `just build` skips; also runs `scripts/check-broker-log-catalog.sh` for forward + reverse hard-FAIL coverage of broker.log event names against the canonical catalog at `.collab/runbooks/broker-log-events.md`), `just test` (full suite), `just test-one -k "pattern"`. `just --list` for everything. Run `just check` before any peer-PASS request or coord cherry-pick batch — it's how peer-PASS verifies "no broken sibling target" and "no undocumented broker.log event"; the coord-side `install-all` does not (#432 receipt: test_relay_enc rotted for ~2 days behind narrow-target builds before Slice E surfaced it). New broker.log event names must be added to the catalog in the same slice; removed events must be deleted from it — both directions hard-FAIL `just check`. Reach for raw `opam exec -- dune build` only if a recipe is missing — if you do, add a recipe.
...
- **`C2C_RELAY_E2E_STRICT_V2`** (Slice C of the CRIT-1+2 plan): when truthy (`1`, `true`, `yes`, `on` — case-insensitive), the relay-e2e verifier rejects envelopes with `envelope_version < 2` before checking the signature. Default off — v1 envelopes continue to verify normally during the v1↔v2 cutover window. The flag is env-read on every verify, so ops can flip it without daemon restart. Used together with Slice B-min-version (per-peer downgrade pin): B handles once-seen-v2-stays-v2 attacks, C handles the global cutover for first-contact peers. The `verify_envelope_sig_detailed` form returns `Verify_err_strict_v2_required { rejected_version }` for ops triage. The GUI (`gui/`) is a CLI wrapper (`Command.create("c2c", ...)`) with no separate verifier — flipping this flag does not require GUI changes. See `.collab/design/2026-04-29-relay-crypto-crit-fix-plan-cairn.md` "Slice C — Strict-mode flip".

...
- **One slice = one worktree. Always.** (Established 2026-04-25 after a cherry-pick chaos cost the coordinator real time.) Before editing code for a slice, create an isolated worktree under `.worktrees/<slice-name>/` branched from `origin/master` (NOT from local master, which may have unmerged peer work). Keep one logical slice per worktree; commit there; route the SHA to peer/coord reviewers. Never mutate the main tree for slice work — main is for review/coord/cherry-pick only. This preserves unrelated dirty state, lets reviewers target stable SHAs without depending on shared-tree state, and prevents the "branch checkout clobbers another agent's WIP" class of failure. After cherry-picks, verify HEAD is on a named branch (`git branch --show-current`); if it shows blank, you're on detached HEAD — `git switch <branch>` or `git switch -c <branch>` before your next commit. See lyra-quill's handoff at `.collab/updates/2026-04-25T09-58-27Z-lyra-coordinator-handoff.md` for the full pattern.

...
- **PTY injection** (deprecated but still useful): `claude_send_msg.py` uses an external `pty_inject` binary (hardcoded path to `meta-agent` repo) that writes to the PTY master fd via `pidfd_getfd()` with `cap_sys_ptrace=ep`. Bracketed paste + 200ms delay + Enter as two writes. **Limitation for Kimi**: prompt_toolkit ignores bracketed-paste sequences when idle, so wake prompts are silently dropped. Use `c2c_pts_inject.py` (direct `/dev/pts/<N>` write) instead for Kimi — routed via `--client kimi` in `c2c_inject.py` and `c2c_deliver_inbox.py`.
...
c2c_pts_inject.py                                                  # Library: direct /dev/pts/<N> write helper. Bypasses bracketed-paste sequences that prompt_toolkit (Kimi) ignores when idle. inject(pts_num, message, crlf=True, char_delay=None). Used by c2c_inject.py and c2c_deliver_inbox.py via --client kimi.
c2c_kimi_wake_daemon.py --terminal-pid P --pts N [--session-id S] [--min-inject-gap N] [--once]  # Auto-delivery for Kimi: watches inbox via inotifywait, injects wake prompt via direct /dev/pts write (c2c_pts_inject.py) to bypass bracketed-paste issue. --client kimi flag routes through pts_inject. Idle delivery fix not yet live-proven.

    ...
    if raw = "" then []
      ...
      | `List items ->
          List.filter_map (fun item ->
              ...
              else Some C2c_mcp.{ from_alias; to_alias; content; deferrable = false; reply_via = None; enc_status = None; ts = 0.0; ephemeral = false; message_id = None; pow_difficulty = None })
            items
      | _ -> []
      | exception _ -> []

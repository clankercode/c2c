...
let test_mem_meta_missing_visibility_fails_closed () =
  (* meta.json parses and has history_public:true but omits visibility ⇒
     incomplete metadata ⇒ fail closed (symmetric with missing history_public). *)
  with_temp_dir (fun dir ->
    write_file (Filename.concat dir "rooms/novis/history.jsonl")
      "{\"message_id\":\"m1\",\"from_alias\":\"alice\",\"content\":\"hi\",\"ts\":1.0}\n";
    write_file (Filename.concat dir "rooms/novis/meta.json")
      "{\"history_public\":true}";
    let t = Relay.InMemoryRelay.create ~persist_dir:dir () in

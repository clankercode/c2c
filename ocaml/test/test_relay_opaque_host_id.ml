...
let test_relay_register_with_alias_embedded_host_id () =
  ...
  check string "alias stored as full form" "lyra-quill#3d08761ae3f3" alias_val;
  let peers = Relay.InMemoryRelay.list_peers ~include_dead:false t in
  check int "one peer" 1 (List.length peers);
  let peer = List.hd peers in
  let peer_alias = Relay.RegistrationLease.alias peer in
  check string "peer alias roundtrip" "lyra-quill#3d08761ae3f3" peer_alias

let test_relay_register_rejects_malformed_alias_with_hash () =
  let t = make_test_relay () in
  let status, _lease = Relay.InMemoryRelay.register t
    ~node_id:"n1" ~session_id:"s1"
    ~alias:"lyra-quill#NotHex" () in
  check string "malformed alias rejected" "invalid_alias" status
  ...
  run "Test_relay_opaque_host_id"
    [ "c2c_name validators", [
      ...
      "Relay lease round-trip", [
        ...
        test_case "register rejects malformed #hostid alias" `Quick

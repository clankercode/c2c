(* c2c_cold_boot_context — cold-boot context shared by Claude hooks
 *
...
let emit_context_json context =
  print_string (Yojson.Safe.to_string (context_json context));
  print_newline ()

let alias_for_session ~broker_root ~session_id =
  try
...
let mark_emitted marker_path ts =
  try
...
let context_for_session ~broker_root ~session_id =
  if session_id = "" || broker_root = "" then None
  else
    let alias = alias_for_session ~broker_root ~session_id in
    if alias = "" then None
    else
      let marker_path = cold_boot_marker_path broker_root session_id in
      if Sys.file_exists marker_path then None
      else
        let ts = iso8601_now () in
        try
          let context = context_block ~alias ~ts in
          mark_emitted marker_path ts;
          Some context
        with e ->
          prerr_endline ("cold_boot_context: " ^ Printexc.to_string e);
          None
...
let run () =
  ...
  (match context_for_session ~broker_root ~session_id with
   | Some context -> emit_context_json context
   | None -> ());
  exit 0
